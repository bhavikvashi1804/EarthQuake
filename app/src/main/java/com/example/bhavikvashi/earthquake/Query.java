...
public class Query
{
        ...
        public static ArrayList<EQdata> extractEarthquakes() {


            ArrayList<EQdata> earthquakes = new ArrayList<>();


            try {


                JSONObject jb1=new JSONObject(SAMPLE_JSON_RESPONSE);
                JSONArray ja1=jb1.getJSONArray("features");
                ...
                for(int i=0;i<ja1.length();i++)
                {
                    ...
                    long date;
                    Double mag;
                    JSONObject cObject=ja1.getJSONObject(i);
                    JSONObject properties=cObject.getJSONObject("properties");
                    mag=properties.getDouble("mag");
                    city=properties.getString("place");
                    date=properties.getLong("time");

                    earthquakes.add(new EQdata(mag,city,date));
